...
async def create_post(post: TransactionPostCreate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
...
async def update_post(post_id: str, post: TransactionPostUpdate, db: Session = Depends(get_db)):
    service = TransactionPostService(db)
